function initPaginaProjectos() {
    ...
    function renderizarTabelaProjectos(projectosFiltrados) {
        ...
        projectosPaginados.forEach((projecto) => {
            ...
            const iconeVer = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 12s3.5-7 10-7 10 7 10 7-3.5 7-10 7S2 12 2 12z"></path><circle cx="12" cy="12" r="3"></circle></svg>`;
                ...
                <tr style="border-bottom: 1px solid #f4f4f5; transition: background-color 0.2s;" onmouseover="this.style.backgroundColor='#fafafa'" onmouseout="this.style.backgroundColor='transparent'">
                    ...
                    <td data-label="Ações" style="padding: 16px; text-align: right;">
                        <div style="display: flex; gap: 8px; justify-content: flex-end; flex-wrap: nowrap;">
                            <button type="button" onclick="window.abrirPedidosDoProjecto('${escaparHtml(projecto.id)}')" title="Ver pedidos do projeto" style="display:flex; align-items:center; justify-content:center; width:32px; height:32px; border-radius:6px; background:rgba(37,99,235,0.1); color:#2563eb; border:none; cursor:pointer; transition:background 0.2s; padding:0;">${iconeVer}</button>
                            <button type="button" onclick="window.abrirEditarProjecto('${escaparHtml(projecto.id)}')" title="Editar" style="display:flex; align-items:center; justify-content:center; width:32px; height:32px; border-radius:6px; background:rgba(229,138,19,0.1); color:var(--aksanti-gold); border:none; cursor:pointer; transition:background 0.2s; padding:0;">${iconeEditar}</button>
                            <button type="button" onclick="window.confirmarEliminarProjecto('${escaparHtml(projecto.id)}', '${escaparHtml(projecto.name)}')" title="Eliminar" style="display:flex; align-items:center; justify-content:center; width:32px; height:32px; border-radius:6px; background:transparent; color:var(--texto-secundario); border:1px solid var(--borda-suave); cursor:pointer; transition:all 0.2s; padding:0;" onmouseover="this.style.background='rgba(239,68,68,0.1)'; this.style.color='#ef4444'; this.style.borderColor='transparent';" onmouseout="this.style.background='transparent'; this.style.color='var(--texto-secundario)'; this.style.borderColor='var(--borda-suave)';">${iconeEliminar}</button>
                        </div>
                    </td>
                </tr>
            `;
    ...
    window.abrirPedidosDoProjecto = function(projectoId) {
        window.location.href = 'pedidos.html?project_id=' + encodeURIComponent(projectoId);
    };
